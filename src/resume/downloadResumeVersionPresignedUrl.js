...
module.exports.handler = async (event) => {
  log.info("Received request to get download URL for resume version", { pathParameters: event.pathParameters, eventContext: event.requestContext });
  ...
  try {
    ...
    const fileName = resumeVersionMetadata.fileName;

    // Generate a presigned URL for downloading the resume with the original filename
    ...
    log.info("Download URL for resume version generated successfully", { versionId, userId });
    ...
    log.error("Error generating download URL for resume version", { error: error.message, stack: error.stack });
    return respondWithError(500, "Could not generate download URL. Please try again later.");
  }
};
